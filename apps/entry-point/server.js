...
const APPS_CONFIG = {
    ...
    views: [
      {
        name: 'Visor (Pantalla)',
        path: '/',
        desc: 'Vista principal en PC (proyecta QR para conectar móvil)',
        hideQr: true,
        primaryBtnText: '🌐 Abrir en LAN',
        primaryBtnTitle: 'Abrir visor en red LAN (para emparejar teléfono)',
      },
      {
        name: 'Control (Teléfono)',
        path: '/telescope.html',
        desc: 'Panel de control con sensores para móvil',
        notice: 'Solo UI · Para vincular con el visor, escanea el QR en la pantalla del visor',
        qrLabel: '📱 QR (Solo UI)',
        qrNotice: 'Este código QR abre únicamente la interfaz del control. Para control sincronizado con el visor, abre el Visor en la red LAN y escanea el QR que aparece en su pantalla.',
      },
    ],
  },
  'kiosk-standalone': {
    ...
    description: 'Versión de museo: Dispositivo único para sensores de orientación y zoom por arduino.',
  ...
  'device-lab': {
    ...
    views: [
      { name: 'Laboratorio de Dispositivos', path: '/', desc: 'Orientación, Cielo y Hardware I/O (navegables dentro de la app)' },
    ],
  },
};
    ...
    for (const [id, app] of Object.entries(this.apps)) {
      ...
      result[id] = {
        ...
        views: app.views.map((v) => ({
          ...
          desc: v.desc,
          path: v.path,
          hideQr: Boolean(v.hideQr),
          primaryBtnText: v.primaryBtnText || null,
          primaryBtnTitle: v.primaryBtnTitle || null,
          notice: v.notice || null,
          qrLabel: v.qrLabel || null,
          qrNotice: v.qrNotice || null,
